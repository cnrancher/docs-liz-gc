...
要升级 **kopilot-crd** 和 **kopilot** Helm Chart，请运行以下命令：
...
[NOTE]
====
如果您通过设置页面配置了模型，请在更新之前将设置值报告给 values 文件。
====
...
. 手动刷新 Liz GC UI Repository。进入 local 集群，点击 **Repositories**，进入 Repositories 列表。如果 Liz GC UI Repository 未处于 Active 状态，请点击下拉菜单中的 **Refresh**，直至该 Repository 变为 Active。
...
+
image::how-tos/openai-config.png[OpenAI 配置屏幕]

=== 通过 Helm 图表选择 OpenAI

使用以下 Helm 值配置从 Agent Helm 图表配置 OpenAI：
...
----
helm upgrade --install --namespace kopilot-system --create-namespace -f values.yaml kopilot ./kopilot-0.6.0.tgz
----
...
* **在 UI 上**： 点击 **Show Advanced** 部分。 输入有效的端点，然后点击 Save。
* **在 Helm 图表中**： 设置 **llm.baseURL** 值。
+
image:how-tos/openai-like-end.png[类似 OpenAI 的端点配置屏幕]

[source,bash,subs="attributes+"]
----
helm upgrade --install --namespace kopilot-system --create-namespace --set llm.baseURL="https://myendpoint.example" kopilot ./kopilot-0.6.0.tgz
----
...
. 点击 Save，代理将重启，这可能需要几秒钟。
+
image::how-tos/gemini-config.png[Gemini 配置屏幕]
...
. 点击 Save，代理将重启，这可能需要几秒钟。
+
image::how-tos/qwen-config.png[Qwen 配置屏幕]

=== 通过 Helm 图表选择 Qwen

使用以下 Helm 值配置来自 Agent Helm 图表的 Qwen：
...
. 点击 Save，代理将重启，这可能需要几秒钟。
+
image::how-tos/deepseek-config.png[Deepseek 配置屏幕]

=== 通过 Helm 图表选择 Deepseek

使用以下 Helm 值配置来自 Agent Helm 图表的 Deepseek：
...
. 点击 Save，代理将重启，这可能需要几秒钟。
+
image::how-tos/mimo-config.png[Mimo 配置屏幕]
...
. 点击 Save，代理将重启，这可能需要几秒钟。
+
image::how-tos/kimi-config.png[Kimi 配置屏幕]
...
. 点击 Save，代理将重启，这可能需要几秒钟。
+
image::how-tos/glm-config.png[GLM 配置屏幕]
...
**安装：** 将以下 `AIAgentConfig` 应用到您的本地群集。
...
----
2. 然后使用 `caBundleRef` 以编程方式配置 **AIAgentConfig**
